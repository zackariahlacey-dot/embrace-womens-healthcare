    ...
    >
      <div className="mx-auto max-w-6xl">
        <div className="text-center mb-12">
          ...
          <motion.p
            className="mt-3 text-sm sm:text-base text-[#3D4A66] font-sans tracking-wide max-w-2xl mx-auto"
            initial={{ opacity: 0 }}
        ...
        <motion.div
          className="mx-auto max-w-3xl mb-12 flex items-center gap-3 rounded-2xl border border-[#3AA5A0]/25 bg-[#3AA5A0]/10 px-5 py-4 shadow-sm"
          initial={{ opacity: 0, y: 10 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.4 }}
        >
          <ShieldCheck className="w-5 h-5 shrink-0 text-[#3AA5A0]" aria-hidden />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 space-y-10 text-left">
            {/* New Patient Visits */}
            <div>
              <div className="flex items-baseline justify-between mb-4 pb-2 border-b border-[#1F2E4C]/15">
                <h3 className="font-serif italic text-xl md:text-2xl text-[#1F2E4C] font-semibold">
                  New Patient Visits
                </h3>
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[#3AA5A0]">
                  First-time
                </span>
              </div>
            ...
            <motion.div
              className="bg-[#EAE5D9]/40 border border-[#1F2E4C]/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-4"
              initial={{ opacity: 0, y: 15 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: 0.3 }}
            >
              <h4 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[#1F2E4C] mb-2">
                <Info className="w-4 h-4 text-[#3AA5A0]" />
                Billing Details & Guidelines
              </h4>
              <ul className="space-y-3">
                {policies.map((policy) => (
                  <li key={policy} className="flex items-start gap-3 text-xs sm:text-sm text-[#3D4A66]">
                    <span className="flex shrink-0 w-5 h-5 rounded-full bg-[#FAF8F5] border border-[#1F2E4C]/10 items-center justify-center text-[#3AA5A0] mt-0.5 shadow-sm">
              ...
              <div className="mt-4 pt-4 border-t border-[#1F2E4C]/10 space-y-2 text-[11px] sm:text-xs text-[#3D4A66]/90 leading-relaxed">
                <p>
                  <span className="font-semibold text-[#1F2E4C]">Payment:</span> Must be paid at the time of service through the patient portal.
                </p>
                <p>
                  <span className="font-semibold text-[#1F2E4C]">Cancellation / No-show fees:</span> Cancellations less than 24 hours before appointment time will be charged $25 for the 1st occurrence, $50 for the 2nd occurrence, and result in termination from the practice on the 3rd occurrence.
                </p>
                <p className="italic">
          ...
          >
            <div className="relative w-full max-w-[360px] aspect-[4/5] overflow-hidden rounded-2xl border-4 border-[#EAE5D9] shadow-lg bg-white group">
              <Image
                src="/womens-healthcare-vermont-transparent-pricing.avif"
...
function PriceCard({ visit, delay }: { visit: VisitPrice; delay: number }) {
  return (
    <motion.div
      className="flex flex-col bg-white border border-[#1F2E4C]/10 rounded-2xl p-5 shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5"
      initial={{ opacity: 0, y: 15 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
    ...
    >
      ...
      <h4 className="font-serif text-base md:text-lg font-semibold text-[#1F2E4C] leading-tight mt-1 mb-3">
        {visit.title}
      </h4>
      <p className="mt-auto font-serif text-2xl md:text-3xl font-semibold text-[#1F2E4C]">
        {visit.price}
      </p>
    </motion.div>
  );
